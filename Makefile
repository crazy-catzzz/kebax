...
# Make parts
make_all: make_drivers make_boot make_stage1 make_stage2 make_pic make_idt make_irq make_pmm make_vmm make_core
...
make_pic:
	$(CC) $(CC_FLAGS) -c $(SOURCE)/boot/pic/*.c -I$(INCLUDE)
	mv -f *.o $(OUT)
...
make_vmm:
	$(CC) $(CC_FLAGS) -c $(SOURCE)/core/vmm/*.c -I$(INCLUDE)
	$(NASM) -f elf32 $(SOURCE)/core/vmm/*.asm
	mv -f $(SOURCE)/core/vmm/*.o $(OUT)
	mv -f *.o $(OUT)
